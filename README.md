...
MP4 -> .SRT subtitle file -> time sync data(.time) & subtitle text data(.docx & .txt) -> user intervention for manual translation using DeepL file translation(translated .docx) or translated .txt using another method -> translated .docx to translated .txt -> join .time and .txt to .srt 
